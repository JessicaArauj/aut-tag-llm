...
from script.python.config import METRICS_FILE  # noqa: E402

st.set_page_config(page_title='Test Metrics Dashboard', layout='wide')
st.title('Test Metrics Overview')
st.caption(
    'Run `python -m script.python.aut` to refresh the metrics before reloading this dashboard.'
)
...
def render_config_snapshot(data: Dict[str, Any]) -> None:
...
st.markdown('---')
render_config_snapshot(metrics)
st.markdown('---')
