...
- `.env.local` holds secrets (HUGGINGFACE_API_KEY, SMTP credentials, etc.) that feed into `script/python/config.py`.
...
- `reporting_utils` writes the Excel output and stores a JSON summary (`inputs/nlp_metrics.json`) that includes backend info, vectorizer scores, and feature-importance data.
...
- `save_dashboard_snapshot.py` merges the metrics JSON + classified Excel to produce the dark-mode HTML snapshot (`output/dashboard_snapshot.html`), useful for quick sharing.
...
- **Automation script**: `robots/aut_tests.sh` ensures `.venv` exists, installs dependencies, runs pytest (producing the JSON report), triggers the snapshot generator, and launches the Streamlit dashboard once tests pass.
- **Dashboard**: `dashboards/test_metrics_dashboard.py` now loads both `inputs/nlp_metrics.json` and `output/test_results.json`, rendering backend metadata, vectorizer accuracy, classification reports/feature importance (when available), and the latest pytest outcomes. This gives stakeholders a consolidated health view after every run.

### Artifacts & Traceability
- `output/tagged_file.xlsx` – final classified dataset.
- `inputs/nlp_metrics.json` – machine-readable metrics consumed by the dashboard.
