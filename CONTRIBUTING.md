...
- Run `python -m script.python.aut` with the backend you’re modifying (`ml`, `bert`, `hf`, `llm`, `local`).
- Add or update tests under `tests/` when applicable.
- Use `python -m compileall script/python` or your preferred linter/formatter before submitting.
